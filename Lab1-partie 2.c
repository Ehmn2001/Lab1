...
uint8_t dataTask6[] = "Task1: task6";
uint8_t dataTask4[] = "Task1: task4";
int flag = 0;
/* USER CODE END 0 */
/**
...
/* USER CODE END MX_GPIO_Init_1 */
 ...
 HAL_GPIO_WritePin(GPIOB,GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5, GPIO_PIN_RESET);
 /*Configure GPIO pin : PC13 */
 GPIO_InitStruct.Pin = GPIO_PIN_13;
 GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
 GPIO_InitStruct.Pull = GPIO_NOPULL;
 HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);
 /*Configure GPIO pins : PB10 PB3 PB4 PB5 PB1 */
 GPIO_InitStruct.Pin = GPIO_PIN_13|GPIO_PIN_14|GPIO_PIN_3|GPIO_PIN_4|GPIO_PIN_5;
...
/**
 * @brief  Function implementing the Task1 thread Green led.
...
void StartTask1(void *argument)
{
 ...
 /* Infinite loop */
 for(;;) {
  if(flag) // Si le bouton est appuyé
   {
     HAL_UART_Transmit(&huart2, dataTask1, sizeof(dataTask1), 1000);
     HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET); //Green led off
     osDelay(17000);
     flag = 0;
   }
   else
   {
     HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_SET); //Green led on
   }
   osDelay(100);
 /* USER CODE END 5 */
 }
...
/**
* @brief Function implementing the Task2 thread,pieton rouge.
* @param argument: Not used
* @retval None
*/
...
void StartTask2(void *argument)
{
 ...
 for(;;)
 {
    if(flag) // Si le bouton est appuyé
    {
	  ...
	  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_5,GPIO_PIN_SET); //pieton led on
	  osDelay(4000); //allumer pendant 4 secondes
	  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_5,GPIO_PIN_RESET); //pieton led off
	  osDelay(10000); //eteint pendant 10 secondes
	  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_5,GPIO_PIN_SET); //pieton led on
	  osDelay(3000); //allume pendant 3 secondes
	  flag = 0;
   }
 else{
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_SET); //pieton led on
...
/**
* @brief Function implementing the Task3 thread Orange led.
* @param argument: Not used
* @retval None
*/
...
void StartTask3(void *argument)
{
 ...
 for(;;)
 {
    if(flag) // Si le bouton est appuyé
    {
     ...
     HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3,GPIO_PIN_SET);//Orange LED ON
     osDelay(3000); //allumee pendant 3s
     HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3,GPIO_PIN_RESET);//Orange LED OFF
     osDelay(12000); //eteint pendant 12s
     HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3,GPIO_PIN_SET);//Orange LED ON
     osDelay(2000); //allumee pendant 2s
     HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3,GPIO_PIN_RESET);//Orange LED OFF
     flag = 0; }
     else{
        HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3,GPIO_PIN_RESET);//Orange LED OFF
...
/**
* @brief Function implementing the Task4 thread Red LED .
* @param argument: Not used
* @retval None
*/
...
void StartTask4(void *argument)
{
 ...
 for(;;)
 {
     ...
     osDelay(3000); //Red LED OFF pendant 3s
     HAL_GPIO_WritePin(GPIOB,GPIO_PIN_14,GPIO_PIN_SET); //Red LED ON
     osDelay(14000);//allumer pendant 14s
     HAL_GPIO_WritePin(GPIOB,GPIO_PIN_14,GPIO_PIN_RESET); //Red LED OFF
	 	 flag = 0;
	 	}
   else{
...
/* USER CODE BEGIN Header_StartTask2 */
/**
* @brief Function implementing the Task2 thread,Green  Pieton led.
* @param argument: Not used
* @retval None
*/
...
void StartTask6(void *argument)
{
 ...
 for(;;)
 {
    if(flag) // Si le bouton est appuyé
    {
   HAL_UART_Transmit(&huart2, dataTask6, sizeof(dataTask6), 1000);
   HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET); //pieton led OFF
   osDelay(4000);//eteint pendant 4s
	  HAL_GPIO_WritePin(GPIOB,GPIO_PIN_13,GPIO_PIN_SET); //pieton led ON
	  osDelay(10000);
   HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET); //pieton led OFF
   osDelay(3000);//eteint pendant 3s
   flag = 0;
   }
 else{
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET); //pieton led OFF
